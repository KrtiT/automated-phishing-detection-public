...
PROFILE = "data/seed-probe-execution-contract-v1.json"
...
def test_policy_fixes_sequential_stages_and_no_retry(api):
    ...
    assert value["partition_reads"]["seed_42_calibration"] == ["validation"]
    assert value["partition_reads"]["probes"] == ["validation"]
    for seed in range(43, 47):
        assert value["partition_reads"][f"seed_{seed}"] == ["train", "validation"]
...
@pytest.mark.parametrize("relative", [PROFILE, METHODS, DEVELOPMENT_METHODS, ACCEPTED])
...
def test_supplement_symlink_is_not_followed(api, checkout):
    path = checkout[0] / ACCEPTED

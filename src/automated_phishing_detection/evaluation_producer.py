...
from .bound_runtime import BoundSession
...
def produce_internal_evidence(
    prepared: PreparedInternal, session: BoundSession
) -> ProducedInternal:
    """Score each validated row once; return nothing if any operation fails.

    Only a fresh, already-open BoundSession may be used. Counts measure full
    ...
    """
    if type(prepared) is not PreparedInternal or type(session) is not BoundSession:
        raise EvaluationProducerError("use typed prepared input and BoundSession")
    state = (
        prepared.records,
    ...
    primary = hypothesis_evaluation.evaluate_primary(
        populations={"internal": population}, audit_windows=WindowCounts(28, 252)
    )
    private = {
        ...
        "bindings.json": _json_bytes(
            {
                ...
                "artifact_hashes": dict(session.models.artifact_hashes),
    ...
    return ProducedInternal(
        rows, population, manifests, primary, counts, private, summary
    )

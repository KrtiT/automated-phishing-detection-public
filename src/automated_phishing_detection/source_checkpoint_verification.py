...
def _membership(rows, identity, expected_count):
    ...
    for ordinal, row in enumerate(rows, 1):
        ...
        _require(
            type(row["source_ordinal"]) is int and row["source_ordinal"] == ordinal
        )
        _require(
            row["record_id"]
        ...
        if row["status"] == "invalid_url_or_domain":
            _require(
                row["canonical_url_sha256"] is None
                and row["registrable_domain"] is None
            )
            continue
        ...
        _require(
            type(digest) is str
            and execution_receipt._SHA256.fullmatch(digest) is not None
        )
        _require(canonical_domains.setdefault(digest, domain) == domain)
        valid[row["record_id"]] = (digest, domain)
    return valid
...
def _manifest(contents, identity, report):
    ...
    groups = {digest for digest, _ in valid.values()}
